...
class ConfigurationError(RuntimeError):
    """Raised when the app has not been given the credentials it needs."""


def _get_client() -> groq.Groq:
    """Create the API client only after checking configuration.

    Keeping this lazy means the Streamlit UI can still start on Render and
    explain exactly what is missing instead of crashing during import.
    """
    api_key = os.environ.get("GROQ_API_KEY", "").strip()
    if not api_key:
        raise ConfigurationError(
            "GROQ_API_KEY is not configured. Add your Groq API key to the "
            "service environment variables and restart the service."
        )
...
def _chat_completion(**kwargs):
    ...
    for model_name in candidates:
        try:
            resp = _get_client().chat.completions.create(model=model_name, **kwargs)
